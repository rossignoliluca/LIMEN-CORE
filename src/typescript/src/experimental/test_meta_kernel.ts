...
function createSessionTelemetry(overrides: Partial<SessionTelemetry> = {}): SessionTelemetry {
  return {
    total_turns: 5,
    avg_depth: 1,
...
function createTurnTelemetry(overrides: Partial<TurnTelemetry> = {}): TurnTelemetry {
  return {
    timestamp: new Date(),
    turn_number: 1,
    input_length: 50,
...
// Test: High delegation reduces power
{
  ...
  const result = applyMetaKernel(session, turn, null);
...
// Test: High agency allows full power
{
  ...
  const result = applyMetaKernel(session, turn, state);
...
// Test: Handshake response - affirmative
{
  ...
  const newState = processHandshakeResponse('yes, continue', state);
...
// Test: Handshake response - negative
{
  ...
  const newState = processHandshakeResponse('no, not now', state);
...
// Test: Italian handshake response
{
  ...
  const newState = processHandshakeResponse('sì, continua', state);
...
{
  const state = createDefaultState();
  const recoveryState = enterRecoveryMode(state);
...
// Test: Recovery mode constrains depth
{
  ...
  const turn = createTurnTelemetry();
  const state = enterRecoveryMode(createDefaultState());
...
{
  const telemetry: ComputedTelemetry = {
    depth_velocity: 0,
    domain_spread: 2,
  ...
  const state = createDefaultState();
  const session = createSessionTelemetry();
  const result = applyMetaKernel(session, null, state);
  
  const invariants = checkMetaKernelInvariants(telemetry, state, result);
...
// Test: Invariant detects recovery violation
{
  ...
  const result = {
    ...
    knob_changes: [],
    power_envelope: {
      depth_ceiling: 'deep' as const, // INVALID in recovery
      dimensions_allowed: ['somatic', 'emotional'] as any[],
      pacing: 'normal' as const,
      l2_mode: 'DEEP' as const,
      time_remaining: 1800,
      turns_remaining: 50,
    },
...
// Test: State persists across turns
{
  ...
  const result1 = applyMetaKernel(session, turn1, null);
  const turn2 = createTurnTelemetry({ turn_number: 2 });
  const result2 = applyMetaKernel(session, turn2, result1.new_state);
