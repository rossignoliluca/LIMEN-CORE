...
function selectMode(field: FieldState): Mode {
  ...
  if (field.goal === 'decide') return 'CONTRACT';
  if (field.goal === 'explore') return 'EXPAND';
  if (field.goal === 'regulate') return 'REGULATE';
  if (field.goal === 'inform') return 'EXPAND';  // will be handled by atmosphere
...
function selectAtmosphere(field: FieldState): Atmosphere {
  ...
  if (
    field.domains.some(d => 
      (d.domain === 'H06_MEANING' || d.domain === 'H07_IDENTITY') && 
      d.salience > 0.5
    ) ||
  ...
  const relationalDomains: HumanDomain[] = [
    'H04_EMOTION', 'H09_ATTACHMENT', 'H10_COORDINATION', 'H11_BELONGING'
  ];
  if (field.domains.some(d => relationalDomains.includes(d.domain as HumanDomain) && d.salience > 0.4)) {
    return 'HUMAN_FIELD';
  }
...
function selectPrimitive(field: FieldState, mode: Mode, atmosphere: Atmosphere): Primitive {
  ...
  const hasLoss = field.domains.some(d => d.evidence?.some(e => 
    /\b(died|death|lost|gone|passed|morto|perso)\b/i.test(e)
  ));
  ...
  switch (mode) {
    case 'REGULATE':
      ...
      if (field.arousal === 'high') return 'P01_GROUND';
      // If shutdown → gentle validate
      if (field.arousal === 'low') return 'P02_VALIDATE';
    ...
    case 'CONTRACT':
      ...
      if (atmosphere === 'DECISION') return 'P05_CRYSTALLIZE';
      // If looping → crystallize
      if (field.loop_count >= 3) return 'P05_CRYSTALLIZE';
      // If delegation → return agency
      if (field.flags.includes('delegation_attempt')) return 'P06_RETURN_AGENCY';
...
function selectDepth(field: FieldState, atmosphere: Atmosphere): Depth {
  // Emergency = surface only
  if (atmosphere === 'EMERGENCY') return 'surface';
  
  // High uncertainty = surface
  if (field.uncertainty > 0.6) return 'surface';
  
  // V_MODE can go deep
  if (atmosphere === 'V_MODE' && field.coherence === 'high') return 'deep';
  
  return 'medium';
}

function selectLength(field: FieldState, atmosphere: Atmosphere): Length {
  // Emergency = minimal
  if (atmosphere === 'EMERGENCY') return 'minimal';
  
  // High arousal = brief
  if (field.arousal === 'high') return 'minimal';
  
  // V_MODE may need more
  if (atmosphere === 'V_MODE') return 'moderate';
  
  return 'brief';
}

function selectPacing(field: FieldState, atmosphere: Atmosphere): Pacing {
  // Emergency = slow (grounding)
  if (atmosphere === 'EMERGENCY') return 'slow';
  
  // V_MODE = slow
  if (atmosphere === 'V_MODE') return 'slow';
  
  // High arousal = slow
  if (field.arousal === 'high') return 'slow';
  
  // Operational = responsive
  if (atmosphere === 'OPERATIONAL') return 'responsive';
  
  return 'normal';
}

function selectTone(field: FieldState, atmosphere: Atmosphere): ToneSpec {
...
function getForbidden(field: FieldState, mode: Mode, atmosphere: Atmosphere): string[] {
...
function getRequired(field: FieldState, mode: Mode, atmosphere: Atmosphere): string[] {
...
export function select(field: FieldState): ProtocolSelection {
