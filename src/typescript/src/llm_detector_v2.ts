...
import { RiskFlags } from './response_plan';
...
export interface LLMDetectorConfig {
  ...
  /** Anthropic API key (from env if not provided) */
  api_key?: string;
...
/**
 * Gating decision - should we call LLM?
 */
...
export class LLMDetectorV2 {
   ...
   *
   * Output includes:
   * - Core EarlySignals fields (risk_flags, metacognitive, contributors)
   * - existential_content (primitive signal)
   * - existential_specificity (Yalom's four givens breakdown)
   * - Reason codes for debugging (llm_detector_called, llm_detector_reason, llm_detector_timeout)
   */
  async contribute(
    message: string,
    language: SupportedLanguage
    ...
    // Step 2: Check gating - should we call LLM?
    const gating = shouldCallLLM(message, regexState, language);

    let classification: RegimeClassification | null = null;
    let usedLLM = false;

    // Step 3: Call LLM if gated and available
    if (gating.should_call_llm && this.client && this.config.enabled) {
      try {
    ...
    // Step 4: Build EarlySignals output
    const signals = this.buildSignals(regexState, classification, gating, usedLLM);
   ...
   *
   * Output includes:
   * - risk_flags.v_mode (derived signal)
  ...
  private buildSignals(
    ...
    if (classification) {
      // Use LLM classification (higher quality)
      risk_flags.v_mode = classification.v_mode.triggered;
      risk_flags.emergency = classification.emergency.triggered;
      risk_flags.crisis = classification.regime === 'crisis' || classification.emergency.triggered;
