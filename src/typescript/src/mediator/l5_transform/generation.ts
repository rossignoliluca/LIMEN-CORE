...
// ============================================
// RESPONSE TEMPLATES BY PRIMITIVE
// ============================================
...
const PRIMITIVE_TEMPLATES: PrimitiveTemplate[] = [
  {
...
function detectLanguage(message: string): 'it' | 'en' {
  ...
  if (/[àèéìòù]/.test(message)) return 'it';
...
function selectTemplate(primitive: Primitive, language: 'it' | 'en'): string {
  ...
  const italianTemplates: string[] = [];
  const englishTemplates: string[] = [];
...
function fillSlots(template: string, field: FieldState, originalMessage?: string): string {
...
function generateReflection(topDomain: string, field: FieldState): string {
...
function verifyConstraints(
  ...
  for (const forbidden of selection.forbidden) {
    switch (forbidden) {
      case 'give_advice':
        if (/\b(dovresti|should|ti consiglio|I suggest)\b/i.test(response)) return false;
        break;
