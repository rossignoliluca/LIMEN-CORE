...
function createTestSelection(overrides = {}) {
    return {
...
function createTestGovernor(overrides = {}) {
    return {
        rules_applied: [],
        effect: {
            atmosphere: null,
...
function createTestMetaKernel() {
    return {
        ...
        new_state: (0, meta_kernel_1.createDefaultState)(),
    };
}
// ============================================
// L2 BLINDNESS TESTS
...
// Test: ExecutionContext has no field information
{
    ...
    const context = (0, l2_execution_1.compileExecutionContext)(field, selection, governor, metaKernel);
...
// Test: verifyL2Blindness passes for valid context
{
    ...
    let pass = false;
    try {
        (0, l2_execution_1.verifyL2Blindness)(context);
        pass = true;
    }
    catch (e) {
        pass = false;
...
{
    const runtime = (0, l2_execution_1.selectRuntime)('EMERGENCY', 'medium', 'DEEP');
...
{
    const runtime = (0, l2_execution_1.selectRuntime)('HUMAN_FIELD', 'high', 'DEEP');
...
{
    const runtime = (0, l2_execution_1.selectRuntime)('HUMAN_FIELD', 'medium', 'MEDIUM');
...
// Test: Surface execution returns template
{
    ...
    const context = (0, l2_execution_1.compileExecutionContext)(field, selection, governor, metaKernel);
    (async () => {
        const result = await (0, l2_execution_1.execute)(context);
...
// Test: All primitives have templates
{
    ...
    for (const p of primitives) {
        if (!l2_execution_1.SURFACE_TEMPLATES[p]) {
            missing.push(p);
        }
    }
...
// Test: All templates are bilingual
{
    ...
    const nonBilingual = [];
    for (const [key, template] of Object.entries(l2_execution_1.SURFACE_TEMPLATES)) {
...
// Test: Context hash is deterministic
{
    ...
    const governor = createTestGovernor();
    const metaKernel = createTestMetaKernel();
    const context1 = (0, l2_execution_1.compileExecutionContext)(field, selection, governor, metaKernel);
    const context2 = (0, l2_execution_1.compileExecutionContext)(field, selection, governor, metaKernel);
    // Same inputs should produce same structure (timestamps will differ)
    const hash1 = (0, l2_execution_1.hashExecutionContext)({ ...context1, timestamp: 'fixed', context_id: 'fixed' });
    const hash2 = (0, l2_execution_1.hashExecutionContext)({ ...context2, timestamp: 'fixed', context_id: 'fixed' });
...
{
    const field = createTestField();
    const selection1 = createTestSelection({ forbidden: ['recommend'] });
    const selection2 = createTestSelection({ forbidden: ['diagnose'] });
    const governor = createTestGovernor();
    const metaKernel = createTestMetaKernel();
    const context1 = (0, l2_execution_1.compileExecutionContext)(field, selection1, governor, metaKernel);
    const context2 = (0, l2_execution_1.compileExecutionContext)(field, selection2, governor, metaKernel);
    const hash1 = (0, l2_execution_1.hashExecutionContext)(context1);
    const hash2 = (0, l2_execution_1.hashExecutionContext)(context2);
...
{
    const cap = l2_execution_1.RUNTIME_CAPABILITIES['L2_SURFACE'];
...
{
    const cap = l2_execution_1.RUNTIME_CAPABILITIES['L2_DEEP'];
...
{
    const runtimes = ['L2_SURFACE', 'L2_MEDIUM', 'L2_DEEP'];
    const allCovered = runtimes.every(r => l2_execution_1.RUNTIME_CAPABILITIES[r] !== undefined);
...
//# sourceMappingURL=test_l2_execution.js.map