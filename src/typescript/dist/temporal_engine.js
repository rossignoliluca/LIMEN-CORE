...
exports.temporalEngine = exports.TemporalEngine = void 0;
// ============================================
...
class TemporalDetector {
            ...
            for (const keyword of patterns.keywords) {
                if (keyword.test(message)) {
                    ...
                    markers.push({
                        ...
                        specificity: this.assessSpecificity(message, type),
                        emotional_weight: this.assessEmotionalWeight(message, type)
                    });
     ...
     */
    detectPressure(message, markers) {
    ...
    assessSpecificity(message, type) {
    ...
    assessEmotionalWeight(message, type) {
        let weight = 0.5;
...
class PatternRecognizer {
     ...
     */
    recognizePatterns(episodes, currentMessage) {
        const patterns = [];
    ...
    detectRecurringTheme(episodes, currentMessage) {
        ...
        if (maxCount >= episodes.length * 0.5) {
            return {
                ...
                instances: episodes
                    .filter(ep => ep.domains_active.includes(maxDomain))
                    .map(ep => ({
                    episode_id: ep.id,
                ...
                domains_affected: [maxDomain]
            };
        }
        return null;
    }
    detectEscalation(episodes) {
...
class CausalMapper {
     ...
     */
    mapCausalLinks(message, episodes, dimensionalState) {
        const links = [];
    ...
    detectCrossDomainCausal(dimensionalState) {
        const links = [];
...
class TemporalEngine {
     ...
     */
    analyze(message, episodes, dimensionalState) {
        ...
        // Map causal links
        const causal_links = this.causalMapper.mapCausalLinks(message, episodes, dimensionalState);
        // Generate insights
        const insights = this.generateInsights(markers, patterns, causal_links, dominant_temporal_frame);
        return {
            markers,
            patterns,
            causal_links,
     ...
     */
    generateInsights(markers, patterns, causalLinks, dominantFrame) {
        const insights = [];
        // Pattern insight
     ...
     */
    generateTemporalResponse(analysis, language = 'en') {
     ...
     */
    requiresImmediateAttention(analysis) {
...
// SINGLETON EXPORT
// ============================================
exports.temporalEngine = new TemporalEngine();
exports.default = exports.temporalEngine;
//# sourceMappingURL=temporal_engine.js.map