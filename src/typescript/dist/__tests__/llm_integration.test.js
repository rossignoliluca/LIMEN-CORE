...
// MOCK DATA BUILDERS
// ============================================
function createMockFieldState(overrides = {}) {
    return {
...
describe('Runtime Selection', () => {
    ...
    it('V_MODE with medium arousal → uses l2Mode', () => {
        const runtime = (0, l2_execution_1.selectRuntime)('V_MODE', 'medium', 'MEDIUM');
        expect(runtime).toBe('L2_MEDIUM');
    });
    it('OPERATIONAL with low arousal + DEEP mode → L2_DEEP', () => {
        const runtime = (0, l2_execution_1.selectRuntime)('OPERATIONAL', 'low', 'DEEP');
...
describe('ExecutionContext Compilation', () => {
    ...
    it('merges forbidden from selection and governor', () => {
        ...
        const governor = createMockGovernor();
        governor.effect.forbidden = ['advise'];
        const metaKernel = createMockMetaKernel();
        const context = (0, l2_execution_1.compileExecutionContext)(field, selection, governor, metaKernel);
...
describe('Runtime Capabilities', () => {
    ...
    it('L2_MEDIUM: 1 LLM call, can generate', () => {
        const cap = l2_execution_1.RUNTIME_CAPABILITIES['L2_MEDIUM'];
...
describeLLM('L2 Execution with LLM', () => {
    beforeAll(() => {
        const status = (0, llm_provider_1.checkLLMAvailability)();
        if (!status.available) {
...
//# sourceMappingURL=llm_integration.test.js.map