...
exports.METAKERNEL_RULES = void 0;
exports.applyMetaKernel = applyMetaKernel;
exports.processHandshakeResponse = processHandshakeResponse;
exports.enterRecoveryMode = enterRecoveryMode;
exports.checkMetaKernelInvariants = checkMetaKernelInvariants;
exports.createDefaultState = createDefaultState;
// ============================================
// CONSTANTS
// ============================================
const ALL_DIMENSIONS = ['somatic', 'emotional', 'relational', 'existential', 'systemic'];
...
function calculateAgencySignal(session) {
    const factors = {
...
function calculatePowerLevel(telemetry, state) {
    let power = 0.5;
    power += 0.3 * telemetry.agency_signal;
    power -= 0.4 * telemetry.delegation_attempts_rate;
    if (state.coherence === 'high')
        power += 0.15;
    if (state.coherence === 'low')
        power -= 0.25;
    if (state.recovery_mode)
        power -= 0.3;
    if (state.deep_mode_active)
        power += 0.2;
    return Math.max(0, Math.min(1, power));
}
function powerLevelToL2Mode(power) {
    if (power < 0.4)
        return 'SURFACE';
    if (power < 0.8)
        return 'MEDIUM';
...
function powerLevelToDepth(power) {
    if (power < 0.4)
        return 'surface';
    if (power < 0.6)
        return 'medium';
...
function powerLevelToDimensions(power) {
    if (power < 0.2)
        return ['somatic'];
    if (power < 0.4)
        return ['somatic', 'emotional'];
    if (power < 0.6)
        return ['somatic', 'emotional', 'relational'];
    if (power < 0.8)
        return ['somatic', 'emotional', 'relational', 'systemic'];
    return ALL_DIMENSIONS;
}
function mostRestrictiveDepth(a, b) {
    return DEPTH_VALUES[a] <= DEPTH_VALUES[b] ? a : b;
}
const METAKERNEL_RULES = [
...
exports.METAKERNEL_RULES = METAKERNEL_RULES;
// ============================================
// HANDSHAKE
// ============================================
...
function checkHandshakeRequired(requestedDepth, state, telemetry) {
    ...
    // - OR agency is low
    return (telemetry.delegation_attempts_rate > 0.2 ||
        telemetry.agency_signal < 0.5);
}
// ============================================
// MAIN FUNCTION
// ============================================
function applyMetaKernel(sessionTelemetry, turnTelemetry, currentState, language = 'en') {
    ...
    for (const rule of METAKERNEL_RULES) {
        if (rule.condition(computed, state)) {
            ...
            for (const [key, value] of Object.entries(changes)) {
                if (JSON.stringify(newKnobs[key]) !== JSON.stringify(value)) {
                    knob_changes.push({
                        knob: key,
                        from: newKnobs[key],
                        to: value,
    ...
    const power_envelope = {
        depth_ceiling: mostRestrictiveDepth(newKnobs.max_depth_allowed, depthFromPower),
        dimensions_allowed: newKnobs.dimensions_enabled.filter(d => powerLevelToDimensions(power_level).includes(d)),
...
function processHandshakeResponse(response, state) {
...
// CRISIS RECOVERY
// ============================================
function enterRecoveryMode(state) {
    return {
        ...state,
        recovery_mode: true,
...
function checkMetaKernelInvariants(telemetry, state, result) {
    const violations = [];
...
exports.default = applyMetaKernel;
//# sourceMappingURL=meta_kernel.js.map