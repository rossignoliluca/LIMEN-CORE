...
exports.checkLLMAvailability = checkLLMAvailability;
...
function mapToAnthropicModel(model) {
    switch (model) {
...
// MAIN LLM FUNCTION
// ============================================
async function callLLM(request, preferredProvider) {
    ...
    if (preferredProvider && config[preferredProvider]) {
        ...
        throw new Error('No LLM API key found. Set OPENAI_API_KEY or ANTHROPIC_API_KEY environment variable.');
    }
...
function buildSystemPrompt(context) {
    ...
    const cultureProfile = types_1.CULTURE_PROFILES[context.language];
