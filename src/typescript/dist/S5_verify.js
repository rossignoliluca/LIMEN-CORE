...
exports.FALLBACK_TEMPLATES = void 0;
exports.verify = verify;
exports.getFallbackOutput = getFallbackOutput;
...
// ============================================
function checkForbiddenActions(output, forbidden) {
    const violations = [];
...
function checkRequiredActions(output, required) {
    const violations = [];
...
function checkConstitutionalInvariants(output) {
    const violations = [];
...
function checkOwnership(output, field, selection) {
    const violations = [];
...
function checkSafety(output, field, selection) {
    const violations = [];
...
// FALLBACK SELECTION
// ============================================
function selectFallback(violations, attemptCount) {
    // Critical constitutional violations → immediate SURFACE (no generation)
    const hasConstitutionalCritical = violations.some(v => v.severity === 'critical' && v.category === 'constitutional');
    if (hasConstitutionalCritical) {
        return 'SURFACE';
    }
    // Critical ownership violations → SURFACE
    const hasOwnershipCritical = violations.some(v => v.severity === 'critical' && v.category === 'ownership');
...
// AUDIT TRAIL
// ============================================
function createAuditEntry(input, violations, passed, fallbackLevel) {
    const entry = {
...
// ============================================
function verify(input, attemptCount = 0) {
    const violations = [];
    // Run all checks
    violations.push(...checkForbiddenActions(input.output.text, input.selection.forbidden));
    violations.push(...checkRequiredActions(input.output.text, input.selection.required));
    violations.push(...checkConstitutionalInvariants(input.output.text));
    violations.push(...checkOwnership(input.output.text, input.field, input.selection));
    violations.push(...checkSafety(input.output.text, input.field, input.selection));
    // Determine result
...
function getFallbackOutput(level, selection, language) {
    if (level === 'STOP') {
        return null;
    }
    if (level === 'PRESENCE') {
        return exports.FALLBACK_TEMPLATES.PRESENCE[language] || exports.FALLBACK_TEMPLATES.PRESENCE.en || "I'm here with you.";
    }
    if (level === 'SURFACE') {
        // Select appropriate surface template based on context
        if (selection.atmosphere === 'EMERGENCY') {
            return exports.FALLBACK_TEMPLATES.SURFACE_GROUND[language] || exports.FALLBACK_TEMPLATES.SURFACE_GROUND.en || "Let's pause for a moment.";
        }
        if (selection.atmosphere === 'V_MODE') {
            return exports.FALLBACK_TEMPLATES.SURFACE_RETURN[language] || exports.FALLBACK_TEMPLATES.SURFACE_RETURN.en || "This is yours to decide.";
        }
        return exports.FALLBACK_TEMPLATES.SURFACE_VALIDATE[language] || exports.FALLBACK_TEMPLATES.SURFACE_VALIDATE.en || "I hear you.";
    }
...
//# sourceMappingURL=S5_verify.js.map